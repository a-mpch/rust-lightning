	...
	L::Target: Logger,
{
	...
	fn fail_htlc_backwards_internal_without_forward_event(&self, source: &HTLCSource, payment_hash: &PaymentHash, onion_error: &HTLCFailReason, failure_type: HTLCHandlingFailureType) -> bool {
		...
		match source {
			...
			HTLCSource::PreviousHopData(HTLCPreviousHopData {
				...
				let htlc_failure_reason = self.get_htlc_failure_from_blinded_failure_forward(blinded_failure, onion_error, incoming_packet_shared_secret, phantom_shared_secret, htlc_id);
				self.fail_htlc_backwards_from_forward(&onion_error, failure_type, &mut push_forward_event, short_channel_id, channel_id, htlc_failure_reason);
			},
			HTLCSource::TrampolineForward { previous_hop_data, incoming_trampoline_shared_secret, .. } => {
				...
				for current_hop_data in previous_hop_data {
					...
					let onion_error = HTLCFailReason::reason(LocalHTLCFailureReason::TemporaryTrampolineFailure, Vec::new());
					let htlc_failure_reason = self.get_htlc_failure_from_blinded_failure_forward(blinded_failure, &onion_error, incoming_packet_shared_secret, &incoming_trampoline_shared_secret, htlc_id);
					self.fail_htlc_backwards_from_forward(&onion_error, failure_type.clone(), &mut push_forward_event, short_channel_id, channel_id, htlc_failure_reason);
				}
			}
		}
		push_forward_event
	...
	fn fail_htlc_backwards_from_forward(
		...
		push_forward_event: &mut bool, short_channel_id: &u64, channel_id: &ChannelId,
		failure: HTLCForwardInfo,
	) {
	...
	fn claim_funds_internal(&self, source: HTLCSource, payment_preimage: PaymentPreimage,
		...
		match source {
			...
			HTLCSource::PreviousHopData(hop_data) => {
				self.claim_funds_from_hop_internal(payment_preimage, forwarded_htlc_value_msat, skimmed_fee_msat, from_onchain, startup_replay, next_channel_counterparty_node_id, next_channel_outpoint, next_channel_id, next_user_channel_id, hop_data);
			},
			HTLCSource::TrampolineForward { previous_hop_data, .. } => {
				for current_previous_hop_data in previous_hop_data {
					self.claim_funds_from_hop_internal(payment_preimage, forwarded_htlc_value_msat, skimmed_fee_msat, from_onchain, startup_replay, next_channel_counterparty_node_id, next_channel_outpoint, next_channel_id, next_user_channel_id, current_previous_hop_data);
				}
			}
		}
	}
